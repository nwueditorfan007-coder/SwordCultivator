...
- 日期：`2026-05-24`
- 状态：飞行测试场景已改为“航向角速度输入 + 八向视觉簇 + V4 程序化骨骼 rig + 油门 + 短滑悬停 + 回锋弧线”原型
...
- `BOOST_SPEED = 760.0`
- `ACCELERATION = 1320.0`
- `BOOST_ACCELERATION = 2300.0`
- `DIRECT_BOOST_ACCELERATION = 3000.0`
- `HOVER_BRAKE = 5200.0`
- `SLIP_BRAKE = 4200.0`
- `SLIP_DURATION = 0.12`
...
原型脚本通过 `eight_way_character_set` 导出选项选择 V1/V2/V3/V4；当前默认使用 V4，运行时按 `V` 可以直接切换。V2 来自 `accepted/` 目录，统一为 `768x768` 透明画布，并按头部宽度约 `86px` 做了一轮人物大小规格化。V3 同样来自 `accepted/` 目录，但改用脸部可见区域作为标准；背向方向使用后脑/发髻核心作为同级替代，所有方向只做等比缩放，不做横纵分开拉伸。

V4 不是传统地面角色八向站姿，也不是趴伏飞行姿态。它继续读取 `visual_heading`，但骨骼姿态只人工维护四个主方向：右、上、左、下。右上、左上、左下、右下不再单独调骨骼点，而是由程序按实际航向在相邻两个主方向之间插值生成；插值前会按屏幕侧重映射 `near/far` 肢体，避免左上、左下把不同身体侧的手脚硬插到一起。八向航向簇仍用于外层飞行读法和 V1-V3 贴图选择；V4 的斜向读法则交给骨骼程序过渡。每个主方向都保持“人站在飞剑上”的御剑姿态：飞剑按航向前进，脚和骨盆锁在剑上方，躯干短窄、腿部更长，整体接近火柴人骨架读法。因此 V4 骨骼版验证的是飞行读感：爬升、下潜、斜飞、回锋压身，而不是角色站在地面看上下左右。
...
- `F2`：显示/隐藏八向人物调参面板
- 面板内可切换 V1/V2/V3/V4 资源集，也可以跟随当前飞行方向自动选中对应方向
- `全局缩放`：当前资源集整体倍率
- `方向缩放`：当前方向单独倍率
- `X/Y 偏移`：当前方向运行时位置微调
...
- `USE_GEMINI_8WAY_CRUISE = true` 时，`visual_heading` 被量化成八方向视觉簇；V1-V3 选图，V4 驱动程序化骨骼姿态
- 八向模式下不再用 `sprite_root.rotation` 整张旋转，也不再用 `render_sign` 镜像左右
